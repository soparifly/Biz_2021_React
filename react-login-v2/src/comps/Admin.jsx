 ...
 * 로그인이 되어 있지 않을 때는 로그인 페이지로 redirect를 수행하도록 할 것
 *
 * 매우 간단한 인증(클라이언트 인증)
 * 1. user state 정보가 있는지 알아보기
 * 2. 있으면 관리자 페이지를 보여주고
 * 3. 없으면 login으로 redirect
 * => user state에 있는 사용자 정보가 정말 server에서 인증한 정상 사용자인지 알 수 있는 방법이 없다
 * 이런 상황이 발생하면 매우 보안에 위험한 상태가 된다
 *
 * 다음과 같은 저라를 수행해야 한다 (서버 인증 저라)
 * 1. 페이지를 선택하면 서버에 query를 전송하여 현재 로그인 된 세션이 있는지 확인하기
 * 2. 로그인 세션이 있으면 세션 정보를 수신하여 다시 user에 담고 페이지를 열어서 보여주기
...
function Admin() {
  ...
  // (문제 : user state 정보가 정말 로그인한 정상사용자인지 알 수 없다)
